...
class TamilSongDetailScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<TamilSongDetailScreen> createState() =>
      _TamilSongDetailScreenState();
}
...
class _TamilSongDetailScreenState extends ConsumerState<TamilSongDetailScreen> {
  ...
  Future<Uint8List> _buildSongPdfBytes({
    ...
    final paragraphs = _paragraphs.isNotEmpty
        ? _paragraphs
        : _splitParagraphs(song.fullLyrics ?? '');
    ...
    final payload = <String, dynamic>{
      ...
      'content': <String, dynamic>{
        'paragraphs': paragraphs
            .asMap()
            .entries
            .map((e) => {'paragraphNumber': e.key + 1, 'text': e.value})
            .toList(),
      },
    };
  ...
  Future<void> _printPdf(TamilSong song, TypographySettings typography) async {
    await _withPdfProgress(() async {
      try {
        final bytes = await _buildSongPdfBytes(
          song: song,
  ...
  Future<void> _downloadPdf(
    ...
    await _withPdfProgress(() async {
      try {
        ...
        final filename =
            '${_sanitizePdfName(normalizeTamil(song.displayName))}.pdf';
            ...
            : AppBar(
                ...
                actions: [
                  IconButton(
                    ...
                    onPressed: () =>
                        _adjustReaderFontSize(typography.fontSize - 2),
                  ),
                  IconButton(
                  ...
                  IconButton(
                    ...
                    tooltip: 'Info',
                    onPressed: () => HelpSheet.show(context, 'tamil_songs'),
                  ),
                    ...
                    onSelected: (val) async {
                      switch (val) {
                        ...
                        case 'share':
                          ...
                          break;
                        case 'download_pdf':
                          _downloadPdf(song, typography);
                          break;
                        case 'print_pdf':
                          _printPdf(song, typography);
                          break;
                      ...
                      children: [
                        Text(
                          ...
                          style: theme.textTheme.headlineMedium?.copyWith(
                            fontWeight: FontWeight.bold,
                            fontSize: typography.titleFontSize + 8,
                            fontFamily: typography.resolvedFontFamily,
                          ),
                        ),
  ...
  Widget _buildNavButton({required bool isNext}) {
    return FutureBuilder<TamilSong?>(
      future: isNext
          ? ref
                .read(tamilSongRepositoryProvider)
                .getNextSong(_currentSongId, sortBy: TamilSongSort.songNo)
          : ref
                .read(tamilSongRepositoryProvider)
                .getPreviousSong(_currentSongId, sortBy: TamilSongSort.songNo),
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  ...
                  Text(
                    neighbor.displayName.length > 15
                        ? '${neighbor.displayName.substring(0, 15)}...'
