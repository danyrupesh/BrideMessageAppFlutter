...
import 'widgets/story_widget_factory.dart';

final _storyImageProvider = FutureProvider.autoDispose
    .family<Uint8List?, (String, String)>((ref, args) async {
      final repo = StoryRepository(DatabaseManager(), args.$1);
      return repo.imageForKey(args.$2);
    });
...
class _StoryReaderScreenState extends ConsumerState<StoryReaderScreen> {
  ...
  Future<Uint8List?> _loadImage(String key) {
    return ref.read(_storyImageProvider((widget.lang, key)).future);
  }
  ...
  String _htmlToPlainText(String html) {
    return html
        ...
        .replaceAll(
          RegExp(
            r'</(p|div|h[1-6]|li|blockquote|br)\s*>',
            caseSensitive: false,
          ),
          '\n\n',
        )
  ...
  List<String> _splitParagraphs(String content) {
    final normalized = _htmlToPlainText(content).replaceAll('\r\n', '\n');
    return normalized
        .split(RegExp(r'\n\s*\n'))
  ...
  String _resolvePdfTitle({required String title, required String langCode}) {
  ...
  Future<Uint8List> _buildEnglishLocalPdfBytes(String title) async {
    final doc = pw.Document();
    final normalizedTitle = _normalizePdfText(title.trim());
  ...
  Future<Uint8List> _buildStoryPdfBytes({
    ...
    return _buildEnglishLocalPdfBytes(title);
  }
  ...
  Future<void> _printPdf({
    ...
    await _withPdfProgress(() async {
      ...
      try {
        bytes = await _buildStoryPdfBytes(
          title: title,
          langCode: langCode,
      ...
      final safeTitle = _sanitizePdfName(
        _resolvePdfTitle(title: title, langCode: langCode),
      );
  ...
  Future<void> _downloadPdf({
    ...
    await _withPdfProgress(() async {
      ...
      final filename =
          '${_sanitizePdfName(_resolvePdfTitle(title: title, langCode: langCode))}.pdf';
  ...
  Widget build(BuildContext context) {
    ...
    return itemAsync.when(
      ...
      data: (item) {
        ...
        final langCode = widget.lang.toLowerCase() == 'ta' ? 'ta' : 'en';
        final typography = ref.watch(typographyProvider(langCode));
        final theme = Theme.of(context);
        final cs = theme.colorScheme;

        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) _syncDocument(item.content);
        });

        final sermonLikeFontSize = typography.fontSize
            .clamp(12.0, 56.0)
            .toDouble();
        final bodyStyle =
        ...
        final appBar = _isSearching
            ? AppBar(
                ...
                title: TextField(
                  ...
                  onChanged: (_) => _syncDocument(item.content),
                  onSubmitted: (_) {
            ...
            : AppBar(
                leading: IconButton(
                  icon: const Icon(Icons.arrow_back),
                  onPressed: () => context.go('/stories?lang=$langCode'),
                ),
                toolbarHeight: 72,
                title: Text(
                  item.title,
                ...
                actions: [
                  IconButton(
                    ...
                    onPressed: () => _adjustReaderFontSize(
                      langCode,
                  ...
                  Padding(
                    ...
                    child: TextButton.icon(
                      onPressed: () => HelpSheet.show(context, 'stories'),
                            ...
                            child: _activeQuery == null
                                ...
                                : Column(
                                    ...
                                    children: [
                                      for (
                                        var i = 0;
                                        i < _paragraphs.length;
                                        i++
                                      )
                                        Padding(
                                          key: _keyForParagraph(i),
                                          padding: EdgeInsets.only(
                                            bottom: i == _paragraphs.length - 1
                                                ? 0
                                                : 14,
                                          ),
                                            ...
                                            TextSpan(
                                              children: _buildHighlightedSpans(
                                                ...
                                                currentOccurrenceIndex:
                                                    _currentOccurrenceForParagraph(
                                                      i,
                                                    ),
                                              ),
                                            ),
